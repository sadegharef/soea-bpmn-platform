...
interface DiagramCardProps {
  ...
  onEditClick: (diagram: Diagram) => void;
}

export const DiagramCard: React.FC<DiagramCardProps> = ({ diagram, viewMode, onMoveClick, onEditClick }) => {
  const { 
    folders, 
    users, 
            ...
            {isManagerOrEditor && (
              <div className="relative">
                <button
                  ...
                  className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-slate-900 dark:hover:text-white transition cursor-pointer"
                >
                ...
                {isMenuOpen && (
                  <div className="absolute left-0 mt-1 w-44 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 py-1 z-20 text-xs">
                    <button
                      onClick={() => { setIsMenuOpen(false); onEditClick(diagram); }}
                      className="w-full text-right px-3 py-2 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-right px-3 py-2 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 flex items-center gap-2 cursor-pointer"
                    >
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500/50 transition shadow-xs hover:shadow-md flex flex-col justify-between overflow-hidden group">
      ...
      <div className="p-4 bg-slate-50/50 dark:bg-slate-900/80 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => openModelerForDiagram(diagram.id)}
            className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition shadow-xs flex items-center gap-1.5 cursor-pointer"
          >
            <ExternalLink className="w-3.5 h-3.5" />
            <span>ورود به مدلساز</span>
          </button>
          ...
          {isManagerOrEditor && (
            <div className="relative">
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-slate-900 dark:hover:text-white transition cursor-pointer"
              >
                <MoreVertical className="w-4 h-4" />
              </button>
