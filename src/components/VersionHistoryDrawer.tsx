 ...
 * @description کشوی جانبی مرور تاریخچه نسخه‌های ذخیره‌شده و بازگردانی نسخه‌های قبلی فرآیند
 * @architecture
 * - Single Responsibility Principle (SRP): نمایش لیست نسخ و کنترل بازیابی نسخه دلخواه
 */
...
interface VersionHistoryDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onRestoreVersionXml: (xml: string) => void;
}
      ...
      >
        ...
        <div className="p-4 bg-slate-950 text-white border-b border-slate-800 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm text-white">تاریخچه نسخه‌ها و تغییرات</h3>
              <p className="text-[11px] text-slate-400 font-mono">آخرین نسخه: v{activeDiagram.latestVersion}.0</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50 dark:bg-slate-900/50">
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 font-medium">
            تمام نسخه‌های ذخیره‌شده برای فرآیند <strong>{activeDiagram.title}</strong>:
          </p>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {isLatest && <span className="text-[10px] bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold px-2 py-0.5 rounded-md">نسخه فعلی</span>}
                  </div>

                  {isEditorOrManager && !isLatest && (
                    <button
                      onClick={() => {
                        if (confirm(`آیا از بازگردانی نسخه ${v.version}.0 اطمینان دارید؟`)) {
                          onRestoreVersionXml(v.xml);
                          onClose();
                        }
                      }}
                      className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 hover:underline bg-indigo-50 dark:bg-indigo-900/30 px-2 py-1 rounded-lg border border-indigo-200/60 dark:border-indigo-800/40"
                    >
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 mt-3 pt-2 border-t border-slate-100 dark:border-slate-700/60">
